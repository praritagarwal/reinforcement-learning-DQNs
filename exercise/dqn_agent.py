...
class Agent():
    ...
    def learn(self, experiences, gamma):
        ...
        estimated_q = self.qnetwork_local(states)[range(BATCH_SIZE), actions.view(-1)].view(-1,1) # there is simpler way to do this by using the .gather() method
        ...
        with torch.no_grad():
            next_q_max = torch.max(self.qnetwork_target(next_states).detach(), axis =1).values.view(-1,1)
